...
void Database::createTables()
{
    if (!m_db.tables().contains("players")) {
        QSqlQuery query("CREATE TABLE players (id INTEGER PRIMARY KEY, firstName TEXT, lastName TEXT)");
        checkQueryStatus(query);
    }

    if (!m_db.tables().contains("competitions")) {
        QSqlQuery query("CREATE TABLE competitions (id INTEGER PRIMARY KEY, tfvbId INTEGER, tournament BOOL, name TEXT, date DATETIME)");
        checkQueryStatus(query);
    }

    if (!m_db.tables().contains("matches")) {
        QSqlQuery query("CREATE TABLE matches (id INTEGER PRIMARY KEY, competition INTEGER, position INTEGER, "
